...
class Enemy(sprite.Sprite):
    ...
    def __init__(self,
        ...
        # This gets set to a player class
        self.goal = None
    ...
    def move(self,
        ...
        if self.isValidLevelMove(xpos, ypos):
            ...
            # Check if we are on the player.
            # If we are, take away a life.
            # NOTE: self.goal is a player object instantiated from a class.
            if self.goal.getX() == xpos:
                if self.goal.getY() == ypos:
                    self.goal.addLives(-1)
    ...
    def simulateAI(self):
        ...
        if self.move(direction.x, direction.y) is False:
